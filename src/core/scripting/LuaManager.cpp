...
namespace YimMenu
{
		...
		{
			{
				...
				std::erase_if(m_LoadedScripts, [this](auto& script) {
					...
					{
						Notifications::Show(
						    "Lua Scripting", std::format("Script {} has been unloaded due to a malfunction", script->GetName()), NotificationType::Warning);
						unload = true;
					}
				    ...
				    else if (script->GetLoadState() == LuaScript::LoadState::WANT_RELOAD && !script->IsMalfunctioning())
				    {
						static std::unordered_map<std::filesystem::path, std::chrono::steady_clock::time_point> reload_cooldown;
						const auto now = std::chrono::steady_clock::now();
						auto& last = reload_cooldown[script->GetPath()];
						if (now - last > std::chrono::seconds(2))
						{
							last = now;
							m_ScriptsToLoad.push(std::string(script->GetPath()));
						}
						unload = true;
				     }
					if (unload)
			...
			if ((*Pointers.CurrentScriptThread)->m_Context.m_ScriptHash != "startup"_J)
			{
	...
	void LuaManager::SetRunningCoroutineImpl(lua_State* script)
	{
		...
		{
			LOGF(FATAL,
			    "LuaManager::SetRunningCoroutineImpl: {} attempted to enter a coroutine when a coroutine from {} is already running",
			    LuaScript::GetScript(script).GetName(),
			    LuaScript::GetScript(m_RunningCoroutine).GetName());
